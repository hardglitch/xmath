...
mod test_func_analysis {
    ...
    fn test_find_extremums2_extremums_pos() {
        ...
        let res = y.find_extremums(-5, 1);
        let mut test_data = HashMap::<String, Point>::new();
        test_data.insert("min".to_owned(), Point{ x: -5.0, y: -333.33 });
        test_data.insert("max".to_owned(), Point{ x: 1.0, y: 10.66 });
