...
impl Im {
    ...
    pub(crate) fn mixed_pow(&self) -> Option<&Self> {
        if let Some(b) = &self.mixed_pow && b.len() == 1 &&
            let Some(e) = b.first()
        {
            return Some(e)
        }
    ...
    pub(crate) fn mixed_pow_mut(&mut self) -> Option<&mut Self> {
    ...
    pub(crate) fn mixed_mul(&self) -> Option<&Self> {
        if let Some(b) = &self.mixed_mul && b.len() == 1 &&
            let Some(e) = b.first()
        {
            return Some(e)
        }
