...
mod test_func_analysis {
    ...
    #[test]
    fn test_find_extrema1_max_min_pos() {
        let mut y = Expression::new(
            |x: f64| 6.0 * x.powf(5.0) - 90.0 * x.powf(3.0) - 5.0
        );
        y.find_extrema(-5.0, 1.0).unwrap();
    ...
    #[test]
    fn test_find_extrema2_extrema_pos() {
        let mut y = Expression::new(
            |x: f64| x.powf(3.0) - 16.0 * x.powf(2.0) / 3.0 + 15.0 * x
        );
        let res = y.find_extrema(-5.0, 1.0).unwrap().unwrap();
    ...
    #[test]
    fn test_find_extrema3_extrema_pos() {
        let mut y = Expression::new(
            |x: f64| x.powf(2.0) + x
        );
        let res = y.find_extrema(-5.0, 1.0).unwrap().unwrap();
        ...
        assert_eq!(Some(vec![min, max]), y.extrema());
    }

    #[test]
    fn test_find_extrema4_extrema_pos() {
        let mut y = Expression::new(
            |x: f64| x.powf(2.0) + x
        );
        let res = y.find_extrema(-5.0, 1.0).unwrap().unwrap();
    ...
    #[test]
    fn test_find_extrema5_none_pos() {
        let mut y = Expression::new(
            |x: f64| (x.powf(3.0) + 1.0) * (x - 1.0) / (x - 2.0).sqrt()
        );
        let res = y.find_extrema(-5.0, 1.0).unwrap();
        assert!(res.is_none());
        assert!(y.extrema().is_none());
    }
